...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        let res = self.entries.iter().find_map(|te| {
            if te.valid && te.asid == asid && te.vpn == vpn {
                return Some(te.ppn);
            } else {
                None
            }
    ...
    pub fn flush_all(&mut self) {
        self.entries.iter_mut().for_each(|te| {
            te.valid = false;
        });
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        let pte = self
            ...
            .iter()
            .find(
                |(asid, mapping)| *asid == self.current_asid && mapping.vpn == vpn, //
            )
