...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for lv in [2usize, 1usize] {
            let vpn = Self::extract_vpn(va_page, lv);
            let pte = self.nodes.get(&cur_ppn).unwrap().entries[vpn];

            if (pte & PTE_V) == 0 {
                let new_ppn = self.alloc_node();
                self.nodes.get_mut(&cur_ppn).unwrap().entries[vpn] = (new_ppn << PPN_SHIFT) | PTE_V;
                cur_ppn = new_ppn;
            } else {
                cur_ppn = pte >> PPN_SHIFT;
            }
        }

        let vpn0 = Self::extract_vpn(va_page, 0);
        let leaf_ppn = pa_page >> PGSHIFT;
        self.nodes.get_mut(&cur_ppn).unwrap().entries[vpn0] =
            (leaf_ppn << PPN_SHIFT) | (flags | PTE_V);
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for lv in [2usize, 1usize, 0usize] {
            let vpn = Self::extract_vpn(va_page, lv);
            let node = self.nodes.get(&cur_ppn);
            let pte = match node {
                None => return TranslateResult::PageFault,
                Some(node) => node.entries[vpn],
            };
            if (pte & PTE_V) == 0 {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                ...
                let page_mask = (1 << (PGSHIFT + (lv as u64) * 9)) - 1;
                let pa = (ppn << PGSHIFT) | (va & page_mask);
        ...
        // 如果 PTE 是叶节点（即 R、W、X 标志位中有至少一个被置位），
        // 则可以直接使用该 PTE 中的物理页号（PPN）计算最终的物理地址。
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte2 = self.nodes.get(&cur_ppn).unwrap().entries[vpn2];

        if (pte2 & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&cur_ppn).unwrap().entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
            cur_ppn = new_ppn;
        } else {
            cur_ppn = pte2 >> PPN_SHIFT;
        }

        let vpn1 = Self::extract_vpn(va, 1);
        let leaf_ppn = pa >> PGSHIFT;
        self.nodes.get_mut(&cur_ppn).unwrap().entries[vpn1] =
            (leaf_ppn << PPN_SHIFT) | (flags | PTE_V);
    }
}
